...
import com.kriptic.app.crypto.EncryptionService
import com.kriptic.app.model.BitchatMessage
import com.kriptic.app.protocol.MessagePadding
import com.kriptic.app.model.RoutedPacket
import com.kriptic.app.model.IdentityAnnouncement
import com.kriptic.app.model.NoisePayload
import com.kriptic.app.model.NoisePayloadType
import com.kriptic.app.protocol.BitchatPacket
import com.kriptic.app.protocol.MessageType
import com.kriptic.app.protocol.SpecialRecipients
import com.kriptic.app.model.RequestSyncPacket
import com.kriptic.app.sync.GossipSyncManager
import com.kriptic.app.util.toHexString
import com.kriptic.app.services.VerificationService
import com.kriptic.app.service.TransportBridgeService
...
class BluetoothMeshService(private val context: Context) : TransportBridgeService.TransportLayer {
    private val debugManager by lazy { try { com.kriptic.app.ui.debug.DebugSettingsManager.getInstance() } catch (e: Exception) { null } }

    companion object {
        private const val TAG = "BluetoothMeshService"
        private val MAX_TTL: UByte = com.kriptic.app.util.AppConstants.MESSAGE_TTL_HOPS
    }
    ...
    private val serviceNotificationManager = com.kriptic.app.ui.NotificationManager(
        context.applicationContext,
        androidx.core.app.NotificationManagerCompat.from(context.applicationContext),
        com.kriptic.app.util.NotificationIntervalManager()
    )
        ...
        gossipSyncManager = GossipSyncManager(
            ...
            configProvider = object : GossipSyncManager.ConfigProvider {
                override fun seenCapacity(): Int = try {
                    com.kriptic.app.ui.debug.DebugPreferenceManager.getSeenPacketCapacity(500)
                } catch (_: Exception) { 500 }

                override fun gcsMaxBytes(): Int = try {
                    com.kriptic.app.ui.debug.DebugPreferenceManager.getGcsMaxFilterBytes(400)
                } catch (_: Exception) { 400 }

                override fun gcsTargetFpr(): Double = try {
                    com.kriptic.app.ui.debug.DebugPreferenceManager.getGcsFprPercent(1.0) / 100.0
                } catch (_: Exception) { 0.01 }
            }
        )

        com.kriptic.app.service.MeshServiceHolder.setGossipManager(gossipSyncManager) { packet ->
    ...
    private fun isBleTransportEnabled(): Boolean {
        return try {
            com.kriptic.app.ui.debug.DebugSettingsManager.getInstance().bleEnabled.value
        } catch (_: Exception) {
            try { com.kriptic.app.ui.debug.DebugPreferenceManager.getBleEnabled(true) } catch (_: Exception) { true }
        }
    }
    ...
    private fun setupDelegates() {
        ...
        peerManager.delegate = object : PeerManagerDelegate {
            override fun onPeerListUpdated(peerIDs: List<String>) {
                ...
                try { com.kriptic.app.services.AppStateStore.setTransportPeers("BLE", peerIDs) } catch (_: Exception) { }
            ...
            override fun onPeerRemoved(peerID: String) {
                ...
                try { com.kriptic.app.services.meshgraph.MeshGraphService.getInstance().removePeer(peerID) } catch (_: Exception) { }
        ...
        messageHandler.delegate = object : MessageHandlerDelegate {
            ...
            override fun updatePeerIDBinding(newPeerID: String, nickname: String,
                ...
                try {
                    com.kriptic.app.favorites.FavoritesPersistenceService.shared.findNostrPubkey(publicKey)?.let { npub ->
                        com.kriptic.app.favorites.FavoritesPersistenceService.shared.updateNostrPublicKeyForPeerID(newPeerID, npub)
                    }
                } catch (_: Exception) { }
                ...
                try {
                    when {
                        message.isPrivate -> {
                            ...
                            if (peer.isNotEmpty()) com.kriptic.app.services.AppStateStore.addPrivateMessage(peer, message)
                        }
                        message.channel != null -> {
                            com.kriptic.app.services.AppStateStore.addChannelMessage(message.channel!!, message)
                        }
                        else -> {
                            com.kriptic.app.services.AppStateStore.addPublicMessage(message)
                        }
                    }
                } catch (_: Exception) { }
                ...
                if (delegate == null && message.isPrivate) {
                    try {
                        ...
                        if (senderPeerID != null) {
                            ...
                            val preview = com.kriptic.app.ui.NotificationTextUtils.buildPrivateMessagePreview(message)
            ...
            override fun handleAnnounce(routed: RoutedPacket) {
                serviceScope.launch {
                    ...
                    if (deviceAddress != null && pid != null) {
                        ...
                        val isDirect = routed.packet.ttl == com.kriptic.app.util.AppConstants.MESSAGE_TTL_HOPS
        ...
        connectionManager.delegate = object : BluetoothConnectionManagerDelegate {
            override fun onPacketReceived(packet: BitchatPacket, peerID: String, device: android.bluetooth.BluetoothDevice?) {
                ...
                try {
                    com.kriptic.app.ui.debug.DebugSettingsManager.getInstance().logIncoming(
                        packet = packet,
                        fromPeerID = peerID,
                        fromNickname = null,
            ...
            override fun onDeviceConnected(device: android.bluetooth.BluetoothDevice) {
                ...
                try {
                    ...
                    com.kriptic.app.ui.debug.DebugSettingsManager.getInstance()
    ...
    fun startServices() {
        ...
        if (!isBleTransportEnabled()) {
            ...
            com.kriptic.app.service.MeshServiceHolder.stopSharedGossip("BLE")
            try { com.kriptic.app.services.AppStateStore.clearTransportPeers("BLE") } catch (_: Exception) { }
            try { com.kriptic.app.services.AppStateStore.clearTransportDirectPeers("BLE") } catch (_: Exception) { }
            return
        }
        if (terminated) {
        ...
        if (connectionManager.startServices()) {
            ...
            com.kriptic.app.service.MeshServiceHolder.startSharedGossip("BLE")
    ...
    fun sendMessage(content: String, mentions: List<String> = emptyList(), channel: String? = null) {
        ...
        serviceScope.launch {
            // FIX: previously this sent raw content bytes as the payload,
            // which silently dropped the `channel` parameter entirely —
            // nothing about which channel a broadcast belonged to ever
            // left this device, so every receiver saw it as an
            // unchanneled public message (or a caller checking
            // `message.channel` on the receiving side always got null).
            // BitchatMessage.toBinaryPayload() already supports encoding
            // channel (see model/BitchatMessage.kt) and is what the
            // private-message path uses via its own structured encoding —
            // use the same structured approach here so channel actually
            // survives the trip. MessageHandler.handleBroadcastMessage()
            // must decode with BitchatMessage.fromBinaryPayload() to match.
            val structuredMessage = com.kriptic.app.model.BitchatMessage(
                sender = delegate?.getNickname() ?: "unknown",
                content = content,
                timestamp = Date(),
            ...
            val encodedPayload = structuredMessage.toBinaryPayload() ?: content.toByteArray(Charsets.UTF_8)
            android.util.Log.d("KripticDebug", "SEND channel=$channel content='$content' encodedBytes=${encodedPayload.size}")
     ...
     */
    fun sendFileBroadcast(file: com.kriptic.app.model.BitchatFilePacket) {
        try {
     ...
     */
    fun sendFilePrivate(recipientPeerID: String, file: com.kriptic.app.model.BitchatFilePacket) {
        try {
            ...
            serviceScope.launch {
                ...
                if (encryptionService.hasEstablishedSession(recipientPeerID)) {
                    try {
                        ...
                        val noisePayload = com.kriptic.app.model.NoisePayload(
                            type = com.kriptic.app.model.NoisePayloadType.FILE_TRANSFER,
                            data = filePayload
                        )
                        ...
                        val packet = BitchatPacket(
                            ...
                            payload = encrypted,
                            signature = null,
                            ttl = com.kriptic.app.util.AppConstants.MESSAGE_TTL_HOPS
                        )
        ...
        serviceScope.launch {
            ...
            if (encryptionService.hasEstablishedSession(recipientPeerID)) {
                try {
                    ...
                    val privateMessage = com.kriptic.app.model.PrivateMessagePacket(
                    ...
                    val messagePayload = com.kriptic.app.model.NoisePayload(
                        type = com.kriptic.app.model.NoisePayloadType.PRIVATE_MESSAGE,
                        data = tlvData
                    )
    ...
    fun sendReadReceipt(messageID: String, recipientPeerID: String, readerNickname: String) {
        serviceScope.launch {
            ...
            val geo = runCatching { com.kriptic.app.services.MessageRouter.tryGetInstance() }.getOrNull()
            val isGeoAlias = try {
                val map = com.kriptic.app.nostr.GeohashAliasRegistry.snapshot()
                map.containsKey(recipientPeerID)
            } catch (_: Exception) { false }
            if (isGeoAlias && geo != null) {
                geo.sendReadReceipt(com.kriptic.app.model.ReadReceipt(messageID), recipientPeerID)
                return@launch
            }

            try {
                // Avoid duplicate read receipts: check persistent store first
                val seenStore = try { com.kriptic.app.services.SeenMessageStore.getInstance(context.applicationContext) } catch (_: Exception) { null }
                ...
                val readReceiptPayload = com.kriptic.app.model.NoisePayload(
                    type = com.kriptic.app.model.NoisePayloadType.READ_RECEIPT,
    ...
    fun sendBroadcastAnnounce() {
        ...
        serviceScope.launch {
            val nickname = try { com.kriptic.app.services.NicknameProvider.getNickname(context, myPeerID) } catch (_: Exception) { myPeerID }
            ...
            try {
                ...
                if (directPeers.isNotEmpty()) {
                    val gossip = com.kriptic.app.services.meshgraph.GossipTLV.encodeNeighbors(directPeers)
                    tlvPayload = tlvPayload + gossip
                }
                // Always update our own node in the mesh graph with the neighbor list we used
                try {
                    com.kriptic.app.services.meshgraph.MeshGraphService.getInstance()
    ...
    private fun getDirectPeerIDsForGossip(): List<String> {
        return try {
            ...
            try { com.kriptic.app.services.AppStateStore.setTransportDirectPeers("BLE", direct) } catch (_: Exception) { }
            val union = try {
                com.kriptic.app.services.AppStateStore.getDirectPeers().ifEmpty { direct }
     ...
     */
    fun getSessionState(peerID: String): com.kriptic.app.noise.NoiseSession.NoiseSessionState {
        ...
        return try {
            ...
            val withRoute = try {
                ...
                if (rec != null && !rec.contentEquals(SpecialRecipients.BROADCAST)) {
                    ...
                    val path = com.kriptic.app.services.meshgraph.RoutePlanner.shortestPath(myPeerID, dest)
