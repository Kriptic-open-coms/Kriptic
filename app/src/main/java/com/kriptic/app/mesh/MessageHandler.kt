...
import com.kriptic.app.sync.PacketIdUtil
...
class MessageHandler(private val myPeerID: String, private val appContext: android.content.Context) {
        ...
        try {
            ...
            when (noisePayload.type) {
                com.kriptic.app.model.NoisePayloadType.PRIVATE_MESSAGE -> {
                    ...
                    val privateMessage = com.kriptic.app.model.PrivateMessagePacket.decode(noisePayload.data)
                ...
                com.kriptic.app.model.NoisePayloadType.FILE_TRANSFER -> {
                    ...
                    if (file != null) {
                        ...
                        val savedPath = com.kriptic.app.features.file.FileUtils.saveIncomingFile(appContext, file)
                        val message = BitchatMessage(
                            id = uniqueMsgId,
                            sender = delegate?.getPeerNickname(peerID) ?: "Unknown",
                            content = savedPath,
                            type = com.kriptic.app.features.file.FileUtils.messageTypeForMime(file.mimeType),
                            timestamp = java.util.Date(packet.timestamp.toLong()),
                            isRelay = false,
                            isPrivate = true,
                            recipientNickname = delegate?.getMyNickname(),
                ...
                com.kriptic.app.model.NoisePayloadType.VERIFY_CHALLENGE -> {
                ...
                com.kriptic.app.model.NoisePayloadType.VERIFY_RESPONSE -> {
    ...
    private suspend fun sendDeliveryAck(messageID: String, senderPeerID: String) {
        try {
            ...
            val ackPayload = com.kriptic.app.model.NoisePayload(
    ...
    private suspend fun handleBroadcastMessage(routed: RoutedPacket) {
        ...
        try {
            ...
            val file = com.kriptic.app.model.BitchatFilePacket.decode(packet.payload)
            if (file != null) {
                if (isFileTransfer) {
    ...
    private fun handleFavoriteNotificationFromMesh(content: String, fromPeerID: String) {
        try {
            ...
            if (noiseKey != null) {
                com.kriptic.app.favorites.FavoritesPersistenceService.shared.updatePeerFavoritedUs(noiseKey, isFavorite)
                if (npub != null) {
                    // Index by noise key and current mesh peerID for fast Nostr routing
                    com.kriptic.app.favorites.FavoritesPersistenceService.shared.updateNostrPublicKey(noiseKey, npub)
                    com.kriptic.app.favorites.FavoritesPersistenceService.shared.updateNostrPublicKeyForPeerID(fromPeerID, npub)
                }
                ...
                val sys = com.kriptic.app.model.BitchatMessage(
                    sender = "system",
