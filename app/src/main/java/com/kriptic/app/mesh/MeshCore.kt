...
import com.kriptic.app.model.PrivateMessagePacket
...
class MeshCore(
    ...
    private fun setupDelegates() {
        peerManager.delegate = object : PeerManagerDelegate {
            override fun onPeerListUpdated(peerIDs: List<String>) {
                try { com.kriptic.app.services.AppStateStore.setTransportPeers(transport.id, peerIDs) } catch (_: Exception) { }
    ...
    fun sendVerifyChallenge(peerID: String, noiseKeyHex: String, nonceA: ByteArray) {
        val payload = NoisePayload(
            ...
            data = com.kriptic.app.services.VerificationService.buildVerifyChallenge(noiseKeyHex, nonceA)
        )
        sendNoisePayloadToPeer(payload, peerID)
    }

    fun sendVerifyResponse(peerID: String, noiseKeyHex: String, nonceA: ByteArray) {
        val tlv = com.kriptic.app.services.VerificationService.buildVerifyResponse(noiseKeyHex, nonceA) ?: return
        val payload = NoisePayload(
    ...
    private fun buildAnnouncementPayload(announcement: IdentityAnnouncement, nickname: String): ByteArray? {
        ...
        try {
            if (directPeersForGossip.isNotEmpty()) {
                tlvPayload += com.kriptic.app.services.meshgraph.GossipTLV.encodeNeighbors(directPeersForGossip)
            }
    ...
    private fun getDirectPeerIDsForGossip(): List<String> {
        return try {
            ...
            try { com.kriptic.app.services.AppStateStore.setTransportDirectPeers(transport.id, localDirect) } catch (_: Exception) { }
            val union = try {
                com.kriptic.app.services.AppStateStore.getDirectPeers().ifEmpty { localDirect }
        ...
        return try {
            val withRoute = try {
                ...
                if (recipient != null && !recipient.contentEquals(SpecialRecipients.BROADCAST)) {
                    ...
                    val path = com.kriptic.app.services.meshgraph.RoutePlanner.shortestPath(myPeerID, destination)
