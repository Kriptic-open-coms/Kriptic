...
import com.kriptic.app.ui.NotificationTextUtils
import com.kriptic.app.mesh.MeshService
import com.kriptic.app.model.BitchatMessage
import com.kriptic.app.model.DeliveryStatus
...
class MeshDelegateHandler(
    ...
    override fun didReceiveMessage(message: BitchatMessage) {
        // Kriptic: markers and SOS ride the normal channel-message envelope
        // on reserved channel names (see map/MarkerBroadcastBridge.kt and
        // sos/SosTrigger.kt) so they get bitchat's existing relay/dedup/TTL
        // for free. Intercept and consume them here, before they'd
        // otherwise render as ordinary chat messages.
        if (com.kriptic.app.mesh.KripticBridgeHolder.handle(message)) {
            return
        }
                ...
                if (state.getJoinedChannelsValue().contains(message.channel)) {
                    ...
                    val viewingGeohash = try {
                        if (channel.startsWith("geo:")) {
                            ...
                            selected is com.kriptic.app.geohash.ChannelID.Location && selected.channel.geohash.equals(geo, ignoreCase = true)
                        } else false
    ...
    private suspend fun processPeerUpdate(mergedPeers: List<String>) {
            ...
            if (isNostrAlias || isNoiseHex) {
                ...
                val canonical = com.kriptic.app.services.ConversationAliasResolver.resolveCanonicalPeerID(
                    ...
                    nostrPubHexForAlias = { alias ->
                        ...
                        if (com.kriptic.app.nostr.GeohashAliasRegistry.contains(alias)) {
                            com.kriptic.app.nostr.GeohashAliasRegistry.get(alias)
                        } else {
                            // Best-effort: derive pub hex from favorites mapping for mesh nostr_ aliases
                            val prefix = alias.removePrefix("nostr_")
                            val favs = try { com.kriptic.app.favorites.FavoritesPersistenceService.shared.getOurFavorites() } catch (_: Exception) { emptyList() }
                            favs.firstNotNullOfOrNull { rel ->
                                rel.peerNostrPublicKey?.let { s ->
                                    runCatching { com.kriptic.app.nostr.Bech32.decode(s) }.getOrNull()?.let { dec ->
                ...
                if (canonical != currentPeer) {
                    ...
                    com.kriptic.app.services.ConversationAliasResolver.unifyChatsIntoPeer(state, canonical, listOf(currentPeer))
                ...
                val favoriteRel = try {
                    ...
                    if (noiseKey != null) {
                        com.kriptic.app.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(noiseKey)
                    } else null
                ...
                if (favoriteRel?.isMutual == true) {
                    ...
                    if (noiseHex != currentPeer) {
                        com.kriptic.app.services.ConversationAliasResolver.unifyChatsIntoPeer(
                            state = state,
        ...
        mergedPeers.forEach { pid ->
            try {
                ...
                val npub = com.kriptic.app.favorites.FavoritesPersistenceService.shared.findNostrPubkey(noiseKey)
                val tempNostrKey: String? = try {
                    if (npub != null) {
                        val (hrp, data) = com.kriptic.app.nostr.Bech32.decode(npub)
    ...
    private fun unifyChatsIntoPeer(targetPeerID: String, keysToMerge: List<String>) {
        com.kriptic.app.services.ConversationAliasResolver.unifyChatsIntoPeer(state, targetPeerID, keysToMerge)
    }
     ...
     */
    fun getPeerInfo(peerID: String): com.kriptic.app.mesh.PeerInfo? {
