package com.kriptic.app.services

import android.content.Context
import android.util.Log
import com.kriptic.app.mesh.MeshService
import com.kriptic.app.model.ReadReceipt
import com.kriptic.app.nostr.NostrTransport
...
class MessageRouter private constructor(
            ...
            val instance = INSTANCE ?: synchronized(this) {
                INSTANCE ?: run {
                    ...
                    MessageRouter(context.applicationContext, mesh, nostr).also { instance ->
                        ...
                        try {
                            com.kriptic.app.favorites.FavoritesPersistenceService.shared.addListener(instance.favoriteListener)
                        } catch (_: Exception) {}
                        INSTANCE = instance
                    }
                }
    ...
    private val favoriteListener = object: com.kriptic.app.favorites.FavoritesChangeListener {
    ...
    fun sendPrivate(content: String, toPeerID: String, recipientNickname: String, messageID: String) {
        ...
        if (com.kriptic.app.nostr.GeohashAliasRegistry.contains(toPeerID)) {
            Log.d(TAG, "Routing PM via Nostr (geohash) to alias ${toPeerID.take(12)}… id=${messageID.take(8)}…")
            val recipientHex = com.kriptic.app.nostr.GeohashAliasRegistry.get(toPeerID)
            if (recipientHex != null) {
                // Resolve the conversation's source geohash, so we can send from anywhere
                val sourceGeohash = com.kriptic.app.nostr.GeohashConversationRegistry.get(toPeerID)
    ...
    fun sendDeliveryAck(messageID: String, toPeerID: String) {
        ...
        if (com.kriptic.app.nostr.GeohashAliasRegistry.contains(toPeerID)) {
            val recipientHex = com.kriptic.app.nostr.GeohashAliasRegistry.get(toPeerID)
            if (recipientHex != null) {
                nostr.sendDeliveryAckGeohash(messageID, recipientHex, try { com.kriptic.app.nostr.NostrIdentityBridge.getCurrentNostrIdentity(context)!! } catch (_: Exception) { return })
                return
            }
        }
    ...
    fun sendFavoriteNotification(toPeerID: String, isFavorite: Boolean) {
        if (mesh.getPeerInfo(toPeerID)?.isConnected == true && mesh.hasEstablishedSession(toPeerID)) {
            val myNpub = try { com.kriptic.app.nostr.NostrIdentityBridge.getCurrentNostrIdentity(context)?.npub } catch (_: Exception) { null }
    ...
    private fun canSendViaNostr(peerID: String): Boolean {
        return try {
            ...
            if (peerID.length == 64 && peerID.matches(Regex("^[0-9a-fA-F]+$"))) {
                ...
                val fav = com.kriptic.app.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(noiseKey)
                ...
                val fav = com.kriptic.app.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(peerID)
                fav?.isMutual == true && fav.peerNostrPublicKey != null
            } else {
                false
            }
