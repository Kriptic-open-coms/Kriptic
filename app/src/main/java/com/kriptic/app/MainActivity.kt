...
import com.kriptic.app.onboarding.BluetoothCheckScreen
import com.kriptic.app.onboarding.BluetoothStatus
import com.kriptic.app.onboarding.BluetoothStatusManager
import com.kriptic.app.onboarding.BatteryOptimizationManager
import com.kriptic.app.onboarding.BatteryOptimizationPreferenceManager
import com.kriptic.app.onboarding.BatteryOptimizationScreen
import com.kriptic.app.onboarding.BatteryOptimizationStatus
import com.kriptic.app.onboarding.BackgroundLocationPermissionScreen
import com.kriptic.app.onboarding.InitializationErrorScreen
import com.kriptic.app.onboarding.InitializingScreen
import com.kriptic.app.onboarding.LocationCheckScreen
import com.kriptic.app.onboarding.LocationStatus
import com.kriptic.app.onboarding.LocationStatusManager
import com.kriptic.app.onboarding.OnboardingCoordinator
import com.kriptic.app.onboarding.OnboardingState
import com.kriptic.app.onboarding.PermissionExplanationScreen
import com.kriptic.app.onboarding.PermissionManager
import com.kriptic.app.ui.ChatScreen
import com.kriptic.app.ui.ChatViewModel
import com.kriptic.app.ui.OrientationAwareActivity
import com.kriptic.app.ui.theme.KripticTheme
import com.kriptic.app.wifiaware.WifiAwareController
import com.kriptic.app.nostr.PoWPreferenceManager
...
class MainActivity : OrientationAwareActivity() {
    ...
    private val forceFinishReceiver = object : android.content.BroadcastReceiver() {
        override fun onReceive(context: android.content.Context, intent: android.content.Intent) {
            if (intent.action == com.kriptic.app.util.AppConstants.UI.ACTION_FORCE_FINISH) {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val filter = android.content.IntentFilter(com.kriptic.app.util.AppConstants.UI.ACTION_FORCE_FINISH)
        if (android.os.Build.VERSION.SDK_INT >= 33) {
            registerReceiver(
                forceFinishReceiver,
                filter,
                com.kriptic.app.util.AppConstants.UI.PERMISSION_FORCE_FINISH,
                null,
        ...
        com.kriptic.app.service.AppShutdownCoordinator.cancelPendingShutdown()
        ...
        meshService = com.kriptic.app.service.MeshServiceHolder.getOrCreate(applicationContext)
        unifiedMeshService = com.kriptic.app.service.MeshServiceHolder.getUnifiedOrCreate(applicationContext)
    ...
    private fun OnboardingFlowScreen(modifier: Modifier = Modifier) {
        ...
        when (onboardingState) {
            ...
            OnboardingState.CHECKING, OnboardingState.INITIALIZING, OnboardingState.COMPLETE -> {
                ...
                com.kriptic.app.ui.nav.KripticAppScaffold(chatViewModel = chatViewModel)
            }
    ...
    private fun checkBluetoothAndProceed() {
        ...
        val bleRequired = try { com.kriptic.app.ui.debug.DebugPreferenceManager.getBleEnabled(true) } catch (_: Exception) { true }
        if (!bleRequired) {
        ...
        lifecycleScope.launch {
            ...
            if (permissionManager.isFirstTimeLaunch()) {
                ...
                if (permissionManager.needsBackgroundLocationPermission() &&
                    ...
                    !com.kriptic.app.onboarding.BackgroundLocationPreferenceManager.isSkipped(this@MainActivity)
                ) {
                    mainViewModel.updateOnboardingState(OnboardingState.BACKGROUND_LOCATION_EXPLANATION)
                } else {
    ...
    private fun handleLocationEnabled() {
        ...
        com.kriptic.app.wifiaware.WifiAwareController.startIfPossible()
        checkBatteryOptimizationAndProceed()
    }
    ...
    private fun handleOnboardingComplete() {
        ...
        val bleRequired2 = try { com.kriptic.app.ui.debug.DebugPreferenceManager.getBleEnabled(true) } catch (_: Exception) { true }
        when {
    ...
    private fun startMeshForegroundServiceBestEffort() {
        ...
        try {
            com.kriptic.app.service.MeshForegroundService.start(applicationContext)
    ...
    private fun initializeApp() {
        ...
        lifecycleScope.launch {
            try {
                ...
                com.kriptic.app.nostr.LocationNotesInitializer.initialize(this@MainActivity)
    ...
    private fun handleNotificationIntent(intent: Intent) {
        val shouldOpenPrivateChat = intent.getBooleanExtra(
            com.kriptic.app.ui.NotificationManager.EXTRA_OPEN_PRIVATE_CHAT, 
            false
        )
        
        val shouldOpenGeohashChat = intent.getBooleanExtra(
            com.kriptic.app.ui.NotificationManager.EXTRA_OPEN_GEOHASH_CHAT,
            false
        )
        
        when {
            shouldOpenPrivateChat -> {
                val peerID = intent.getStringExtra(com.kriptic.app.ui.NotificationManager.EXTRA_PEER_ID)
                val senderNickname = intent.getStringExtra(com.kriptic.app.ui.NotificationManager.EXTRA_SENDER_NICKNAME)
            ...
            shouldOpenGeohashChat -> {
                val geohash = intent.getStringExtra(com.kriptic.app.ui.NotificationManager.EXTRA_GEOHASH)
                ...
                if (geohash != null) {
                    ...
                    val level = when (geohash.length) {
                        7 -> com.kriptic.app.geohash.GeohashChannelLevel.BLOCK
                        6 -> com.kriptic.app.geohash.GeohashChannelLevel.NEIGHBORHOOD
                        5 -> com.kriptic.app.geohash.GeohashChannelLevel.CITY
                        4 -> com.kriptic.app.geohash.GeohashChannelLevel.PROVINCE
                        2 -> com.kriptic.app.geohash.GeohashChannelLevel.REGION
                        else -> com.kriptic.app.geohash.GeohashChannelLevel.CITY // Default fallback
                    }
                    val geohashChannel = com.kriptic.app.geohash.GeohashChannel(level, geohash)
                    val channelId = com.kriptic.app.geohash.ChannelID.Location(geohashChannel)
