package com.kriptic.app.service
...
import com.kriptic.app.MainActivity
...
class MeshForegroundService : Service() {
    ...
    companion object {
        ...
        const val ACTION_START = "com.kriptic.app.service.START"
        const val ACTION_STOP = "com.kriptic.app.service.STOP"
        const val ACTION_QUIT = "com.kriptic.app.service.QUIT"
        const val ACTION_UPDATE_NOTIFICATION = "com.kriptic.app.service.UPDATE_NOTIFICATION"
        const val ACTION_NOTIFICATION_PERMISSION_GRANTED = "com.kriptic.app.action.NOTIFICATION_PERMISSION_GRANTED"
    ...
    private val unifiedMeshService: com.kriptic.app.mesh.MeshService?
            ...
            updateJob = scope.launch {
                while (isActive) {
                    ...
                    if (eligible) {
                        ...
                        if (isInForeground) {
                            try { if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) stopForeground(STOP_FOREGROUND_REMOVE) else stopForeground(true) } catch (_: Exception) { }
                            isInForeground = false
                        }
                        notificationManager.cancel(NOTIFICATION_ID)
                    }
    ...
    private fun ensureMeshStarted() {
        ...
        val bleEnabled = try {
            com.kriptic.app.ui.debug.DebugPreferenceManager.getBleEnabled(true)
        } catch (_: Exception) {
            true
        }
        if (!bleEnabled) {
    ...
    private fun updateNotification(force: Boolean) {
        ...
        if (MeshServicePreferences.isBackgroundEnabled(true) && hasAllRequiredPermissions()) {
            try {
                ...
                android.util.Log.e("MeshForegroundService", "Failed to update notification: ${e.message}")
            }
        } else if (force) {
    ...
    private fun buildNotification(activePeers: Int): Notification {
        ...
        return NotificationCompat.Builder(this, CHANNEL_ID)
            ...
            .addAction(
                ...
                getString(R.string.notification_action_quit_Kriptic),
                quitPendingIntent
            )
            .build()
    }
