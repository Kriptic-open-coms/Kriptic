...
import com.kriptic.app.services.SeenMessageStore
import com.kriptic.app.ui.ChatState
import com.kriptic.app.ui.MeshDelegateHandler
import com.kriptic.app.ui.PrivateChatManager
...
class NostrDirectMessageHandler(
    ...
    private val dataManager: com.kriptic.app.ui.DataManager
) {
    ...
    fun onGiftWrap(giftWrap: NostrEvent, geohash: String, identity: NostrIdentity) {
        scope.launch(Dispatchers.Default) {
            try {
                ...
                if (packet.type != com.kriptic.app.protocol.MessageType.NOISE_ENCRYPTED.value) return@launch
                ...
                com.kriptic.app.nostr.GeohashAliasRegistry.put(convKey, senderPubkey)
        ...
        when (payload.type) {
            ...
            NoisePayloadType.FILE_TRANSFER -> {
                ...
                if (file != null) {
                    ...
                    val savedPath = com.kriptic.app.features.file.FileUtils.saveIncomingFile(application, file)
                    val message = BitchatMessage(
                        id = uniqueMsgId,
                        sender = senderNickname,
                        content = savedPath,
